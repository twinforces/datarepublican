(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ? factory(
        ...
        require("d3-shape"),
        require("elementary-circuits-directed-graph")
      )
  ...
  findCircuits =
    findCircuits && findCircuits.hasOwnProperty("default")
      ? findCircuits["default"]
      : findCircuits;
  ...
  function sankeyCircular() {
    ...
    function computeNodeBreadths(graph, iterations, id) {
      var columns = Array.from(
        d3.group(graph.nodes, (d) => d.column),
        ([key, values]) => values
      ).sort((a, b) => d3.ascending(a[0].column, b[0].column));
          ...
          nodes.forEach(function (node) {
            ...
            if (node.sourceLinks.length || node.targetLinks.length) {
              if (
                ...
                var avgTargetY = d3Array.mean(
                  node.sourceLinks,
                  linkTargetCenter
                );
                var avgSourceY = d3Array.mean(
                  node.targetLinks,
                  linkSourceCenter
                );
  ...
  // elementary circuits via grok

  function findCircuits(adjList) {
    const cycles = [];
    ...
    function dfs(node) {
      ...
      // Get neighbors (nodes that the current node links to)
      ...
      for (const neighbor of neighbors) {
        if (!visited.has(neighbor)) {
          ...
          if (cycleStart !== -1) {
            // Extract the cycle (from neighbor back to neighbor)
