...
GRANTEE_ADDRESS_XPATHS = [
    ...
    etree.XPath(".//*[local-name()='RecipientForeignAddress']/* | .//*[local-name()='CountryCd']", namespaces={}),  # Handle default namespace
]
ZIP_REGEX = re.compile(r'^\d{5}$')
PO_BOX_REGEX = re.compile(r'P(?:.*?\bBOX\b\s+)([-\w\d]+)', re.IGNORECASE) #first word thing after the last BOX
...
def main():
    ...
    global results_queue, grants_queue
    ...
    try:
        ...
        # Compute ZIP checksums once
        current_checksums = compute_zip_checksums(args.zip_dir)

        # Step 1: Load or build ZIP index cache
        zip_cache_valid, zip_cached_data = load_zip_cache(args.cache_dir, args.start_year, args.end_year, args.zip_dir,current_checksums)
        if zip_cache_valid and not args.force_reprocess:
            zip_index = zip_cached_data
            log_error("Loaded ZIP index cache: {} XML files", len(zip_index))
        else:
            ...
            save_zip_cache(args.cache_dir, args.start_year, args.end_year, current_checksums, zip_index)

        # Step 2: Load or build address and PO box cache
        addr_cache_valid, addr_cached_data = load_address_cache(args.cache_dir, args.start_year, args.end_year, args.zip_dir)
        if addr_cache_valid and not args.force_reprocess:
            ...
            log_error("Loaded address cache: {} addresses, {} PO boxes, {} ZIP index entries", len(address_entries), len(po_box_entries), len(zip_code_index))
        else:
            process_all_xml_addresses(args.worker_threads, zip_index, args.output_dir)
            save_address_cache(args.cache_dir, args.start_year, args.end_year, address_entries, debug_address_entries, po_box_entries, zip_code_index, po_box_zip_index)

        # Step 3: Process charities from charity_latest.tsv
...
def compute_name_heuristic(grantee_name, filer_name):
    ...
    if total_grantee_words == 0:
        return 0
    # Calculate the proportion and scale to a 0-100% range
...
def canonicalize_address(address_components, output_dir):
    ...
    if not address_parts:
        ...
        return "", None, None, ""
    canonical = " ".join(address_parts).title()  # Capitalize words
    for abbr, full in USPS_FIXES.items():  # Apply USPS abbreviations
        canonical = canonical.replace(f' {abbr} ', f' {full} ')
    if state and state.upper() not in VALID_STATES:  # Validate state
            ...
            for xpath_grant in GRANT_XPATHS_990PF:
                ...
                for element in grant_elements:
                    ...
                    for xpath_address in GRANTEE_ADDRESS_XPATHS:
                        ...
                        if addrs:
                            address_elems = addrs
                            if b"Foreign" in xml_content:  # Grep raw XML for Foreign
                                log_error("Found 'Foreign' in XML {}: {}", xml_filename, xml_content[:500].decode('utf-8', errors='ignore'))
                    ...
                    if is_foreign_address:
                        ...
                        if possibly_foreign:
                            log_error("Foreign address no tag? {}", elem.tag for elem in address_elems)
                    ...
                    if grantee_canonical_address or amount_elem is not None or name_elem is not None or len(ein_elem) > 0 or grant_ein != "Unknown":
                        try:
                            ...
                            if is_foreign_address:
                                ...
                                if best_filer:
                                    if best_score >= 50: # 50% of the words have to match
...
def process_all_xml_addresses(worker_threads, zip_index, output_dir):
    global total_addresses, total_address_errors, total_queue_puts, results_queue
    ...
    def process_address_row(row):
       ...
       if not xml_path:
           log_error("No xml_name, skipping")
       ...
       try:
           ...
           if xml_filename not in zip_index:
               log_error("No file {} in ZIP index, skipping (total missing: {})", xml_filename, total_skipped + 1)
               thread_local.result['debug_address_entries'].append({
    ...
    try:
        ...
        log_error("Error in process_all_xml_addresses: {}", str(e), exc_info=True)
        raise
    finally:
         ...
         try:
             with open(os.path.join(output_dir, 'inferred_grants.tsv'), 'w', encoding='utf-8', newline='') as f:
                 ...
                 with tqdm(total=total_grants, desc="Writing inferred_grants.tsv") as pbar:
                     ...
                     log_error("Wrote {} unique grants to inferred_grants.tsv", written_grants)
         except Exception as e:
    ...
    try:
        ...
        if args.no_threads:
            ...
            for xml_filename, filer_info in tqdm(filer_eins.items(), desc="Processing grants"):
                if not filer_info[1]['form_type'] == '990PF':  # Only process 990PF grants
                    continue
        ...
        log_error("Error in process_charity_rows: {}", str(e), exc_info=True)
        raise
    finally:
...
def load_zip_cache(cache_dir, start_year, end_year, zip_dir, current_checksums):
    cache_valid = False
    cached_data = None
...
def load_address_cache(cache_dir, start_year, end_year, zip_dir, current_checksums):
    cache_valid = False
    cached_data = None
    checksum_file = os.path.join(cache_dir, f'zip_checksums_{start_year}_{end_year}.json')
    ...
    if os.path.exists(checksum_file):
        try:
            with open(checksum_file, 'r') as f:
                cached_checksums = json.load(f)
                addresses_file = os.path.join(cache_dir, f'cached_addresses_{start_year}_{end_year}.pkl')                mappings_file = os.path.join(cache_dir, f'cached_mappings_{start_year}_{end_year}.pkl')
