...
Optimized for speed - only does expensive matching on the true uncovered tail.
"""
...
def clean_name(name: str) -> str:
    """Simplified but faithful version of generator's clean_name."""
...
def analyze_name(name: str, rules: Dict, all_variants: set, priority_canonicals: List[str]) -> Dict:
    cleaned = clean_name(name)
    if cleaned in all_variants or name.upper() in all_variants:
        return {
            "raw": name,
            "cleaned": cleaned,
            ...
            "similarity": 1.0,
            "ein": "N/A",
            "reason": "Covered by existing rule/variant"
        }
    ...
    # Only test against priority/SIMPLES canonicals (fast)
    for canon in priority_canonicals:
        ...
        if score > best_score:
            best_score = score
            best_match = canon
            # Try to find EIN if available in rules
    ...
    if best_score > 0.5:
        ...
        reason = f"No good match (best score {best_score:.3f}). Consider adding to SIMPLES."
...
def main():
    ...
    for i, (name, count, dollars) in enumerate(names, 1):
        analysis = analyze_name(name, rules, all_variants, priority_canonicals)
        if "Covered by existing rule" not in analysis["reason"]:
