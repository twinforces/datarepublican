...
## Development Rules
* read https://gist.github.com/twinforces/ab9b8f303504de932b0b85558b4c0a22 for general instructions
...
* Remember that the live database has data, but any test database you generate will be empty.
* there are some test XML files in ./990tools/test_xmls/ The ones beginning with FAIL are intentionally broken.
* Don't use fake addresses. If you must extract some addresses from the test_xmls. 
* This document lives in ./990tools/docs/Architecture.md and can be added to as necessary.
...
* Immutable architecture facts
	...
	* That said, logic can drive the threading for each stage:
		* irsfetch - The only thing where multiple threads make sense is for the download of multiple zip files, which doesn't happen that often.
		...
		* xml - this is the true multithreaded beast. Decompressing the data from the zip file dominates, then it needs to be parsed.
		* address - this is pure duckdb, it sets up the master addresses and creates geocoding objects to set the colocator fields
...
* Global considerations:
	...
	* for speed "quiet" is important on debug and info logging so that if quiet is turned on, it won't even bother to prepare for making the log, so you will see numerous if not global_config.quiet statements in the code. 
...
* While normally CRUD applies for a project like this, something I've learned from enterprise is an Anti-pattern, so there's only Update, to mark something as deleted. Which is just CRU. Also, we use uuid7 Primary and foreign keys exclusively, so we can build entire object trees in memory and then write them in bulk at our convenience. 
...
## Performance notes:
	* Just like each step has its own best thread count, every step has its own unique optimum batch_size which is seperate and should not be confused with max_files even though they interrelate. We will tweak those per step later, but haven't worried about it now, except the geocoding API can only handle 10000 addresse at a time, and we'll probably lower that to 1000 to make the progress information come more smoothly. 
